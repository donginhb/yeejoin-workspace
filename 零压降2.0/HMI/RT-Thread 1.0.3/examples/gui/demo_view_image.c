...
#else
	view = rtgui_filelist_view_create(workbench, "/img", "*.*", &rect);
#endif

	ret = rtgui_view_show(RTGUI_VIEW(view), RT_TRUE);
	printf_syn("fun:%s, line:%d, ret:%d, 0x%x\n", __FUNCTION__, __LINE__, ret, view);

	/* 模态显示一个文件列表视图，以提供给用户选择图像文件 */
	if (RTGUI_MODAL_OK == ret) {
		...
		if (image_type[0] != '\0')
			image = rtgui_image_create_from_file(image_type, path, RT_FALSE); //RT_TRUE); //david

		printf_syn("fun:%s, line:%d, image:0x%x\n", __FUNCTION__, __LINE__, image);
	}
