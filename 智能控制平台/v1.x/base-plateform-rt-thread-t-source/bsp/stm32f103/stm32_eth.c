...
#ifdef ETH_DEBUG
#define STM32_ETH_TRACE(x)	      //printf_syn x
#else
...
#define RX_TX_BUF_SIZE_MAX 1518
static rt_uint8_t rx_buf[ETH_RXBUFNB][RX_TX_BUF_SIZE_MAX];
static rt_uint8_t tx_buf[ETH_TXBUFNB][RX_TX_BUF_SIZE_MAX];
...
rt_err_t rt_stm32_eth_tx( rt_device_t dev, struct pbuf* p)
{
    ...
    for (q = p; q != NULL; q = q->next)
    {
        ...
        #else
        #error "ETH_PAD_SIZE configuration is incorrect!"
        #endif
...
struct pbuf *rt_stm32_eth_rx(rt_device_t dev)
{
    ...
    {
        /* Get the Frame Length of the received packet: substruct 4 bytes of the CRC */
        framelength = ((DMARxDescToGet->Status & ETH_DMARxDesc_FL) >> ETH_DMARXDESC_FRAME_LENGTHSHIFT) - 4 + ETH_PAD_SIZE;
...
/* interrupt service routine for enc28j60 */

/* PKTIF: Receive Packet Pending Interrupt Flag bit */
#define PKTIF_BIT (1<<6)

/* TXIF:Transmit Interrupt Flag bit */
#define TXIF_BIT  (1<<3)

#if 1
volatile unsigned int enc28j60_int_cnt_rx;
volatile unsigned int enc28j60_int_cnt_tx;

volatile unsigned int enc28j60_pkt_cnt_rx;
volatile unsigned int enc28j60_pkt_cnt_tx;
#endif

#define ENABLE_DISABLE_28J60_INT 1

extern unsigned char * get_free_rx_buf(void);
extern int release_free_rx_buf(unsigned char *buf);

/*
 * spi的使用会产生竞争条件, comment by David
 * 
 * 使用enc28j60_send_packet()/enc28j60_receive_packet()时, 关闭stm32对enc28j60的中断响应, 以避免spi的竞争条件
 *
 * 当EPKTCNT减为0时, EIR.PKTIF将会自动清零
 */
void isr_proc_enc28j60_int(void)
{
	int temp, cnt;

#if ENABLE_DISABLE_28J60_INT
	/* disenable interrupt, David  */
	enc28j60_clear_bits(EIE, (1 << EIE_INTIE));
#endif
	temp = enc28j60_read(EIR);
	cnt  =  enc28j60_read(EPKTCNT);/* DS80349C.errata #6, David */
	/* packet receiption */
	if (is_bit_set(temp, PKTIF_BIT)	|| cnt) {
		/* a frame has been received */
		eth_device_ready(&(stm32_eth_device.parent));
		++enc28j60_int_cnt_rx;
	}
	...
	/* packet transmission */
	if (is_bit_set(temp, TXIF_BIT)) {
		enc28j60_clear_bits(EIR, (1 << EIR_TXIF) );
		++enc28j60_int_cnt_tx;
	}

#if ENABLE_DISABLE_28J60_INT
	/* enable interrupt, David  */
	if (0==cnt || is_bit_set(temp, TXIF_BIT))
		enc28j60_set_bits(EIE, (1 << EIE_INTIE));
#endif
...
static rt_err_t rt_stm32_eth_init(rt_device_t dev)
{
	STM32_ETH_TRACE(("func:%s, line:%u!\n", __FUNCTION__, __LINE__));

	rt_sem_init(&enc28j60_spi_free, "spi4eth", 1, RT_IPC_FLAG_FIFO);
	enc28j60_init((u8*)&stm32_eth_device.dev_addr[0]);
...
rt_err_t rt_stm32_eth_tx( rt_device_t dev, struct pbuf* p)
{
	struct pbuf* q;
	rt_err_t result;
	rt_uint8_t *ptr, *ptx_buf;
	rt_uint32_t len, byte_cnt;
	...
	/* get enc28j60 spi */
	result = rt_sem_take(&enc28j60_spi_free, get_ticks_of_ms(WAIT_TXRX_BUF_TIME));
	if (result != RT_EOK) {
		printf_syn("%s() get enc28j60_spi_free fail. system will reset enc28j60!\n", __FUNCTION__);
		reset_whole_system();
		return -RT_ERROR;
	}
	enc28j60_nvic_cfg(DISABLE);
	enc28j60_send_packet(tx_buf[0], byte_cnt);
...
struct pbuf *rt_stm32_eth_rx(rt_device_t dev)
{
	static int continue_proc_cnt;
	int i;
	rt_size_t len, byte_cnt;
	rt_err_t result;

	struct pbuf* p;
	struct pbuf* q;
	rt_uint8_t *ptr, *prx_buf, *prx_buf_o;
	...
	byte_cnt = enc28j60_receive_packet(prx_buf_o, RX_TX_BUF_SIZE_MAX);
	if (0 == byte_cnt) {
		p = NULL;
		/* enable interrupt, David  */
		enc28j60_set_bits(EIE, (1 << EIE_INTIE));
	...
	++enc28j60_pkt_cnt_rx;
	/* allocate buffer */
	p = pbuf_alloc(PBUF_LINK, byte_cnt, PBUF_RAM);
	if (NULL == p) {
		/* NOTE: 将会丢掉接收到的数据包!!!! */
		printf_syn("%s() pbuf_alloc fail!\n", __FUNCTION__);
		/* enable interrupt, David  */
		enc28j60_set_bits(EIE, (1 << EIE_INTIE));
...
#if ENABLE_DISABLE_28J60_INT
	if (0 == enc28j60_read(EPKTCNT) || continue_proc_cnt>16) {
		continue_proc_cnt = 0;
		/* enable interrupt, David  */
		enc28j60_set_bits(EIE, (1 << EIE_INTIE));
	} else {
...
#endif
	prx_buf = prx_buf_o;
	for (q = p; RT_NULL!=q && 0!=byte_cnt; q = q->next) {
		ptr = q->payload;
		len = q->len;
...
void rt_hw_stm32_eth_init()
{
    ...
    stm32_eth_device.dev_addr[0] = MACOCT0;
    stm32_eth_device.dev_addr[1] = MACOCT1;
    stm32_eth_device.dev_addr[2] = MACOCT2;
    // generate MAC addr from 96bit unique ID (only for test)
    stm32_eth_device.dev_addr[3] = MACOCT3;
    stm32_eth_device.dev_addr[4] = MACOCT4;
    stm32_eth_device.dev_addr[5] = MACOCT5;
    ...
    //rt_sem_init(&rx_buf_free, "rx_buf", ETH_RXBUFNB, RT_IPC_FLAG_FIFO);
...
#if 0
static rt_uint32_t rx_buf_used_vector;
/*
 * 返回值是申请到的缓冲区起始地址
 */
unsigned char * get_free_rx_buf(void)
{
	int i;
	rt_err_t result;

	result = rt_sem_take(&rx_buf_free, get_ticks_of_ms(WAIT_TXRX_BUF_TIME));
	if (result != RT_EOK) {
		printf_syn("%s() fail, err:%d!\n", __FUNCTION__, result);
		goto fail_ret;
	}

	for (i=0; i<ETH_RXBUFNB; ++i) {
		if (is_bit_clr(rx_buf_used_vector, 1<<i))
			break;
	}

	if (i < ETH_RXBUFNB) {
		set_bit(rx_buf_used_vector, 1<<i);
		return rx_buf[i];
	}

fail_ret:
	printf_syn("take rx_buf_free sem fail!\n");
	return NULL;
}

int release_free_rx_buf(unsigned char *buf)
{
	int i;
	...
	if (i < ETH_RXBUFNB) {
		clr_bit(rx_buf_used_vector, 1<<i);
		rt_sem_release(&rx_buf_free);
	} else {
		printf_syn("%s() fail!\n", __FUNCTION__);
		return FAIL;
	}
...
//#if 0
int reset_en28j60_driver(void)
{
	printf_syn("system will reset enc28j60!\n");

	rt_sem_control(&tx_buf_free, RT_IPC_CMD_RESET, (void *)ETH_TXBUFNB);
	rt_sem_control(&rx_buf_free, RT_IPC_CMD_RESET, (void *)ETH_RXBUFNB);
	rt_sem_control(&enc28j60_spi_free, RT_IPC_CMD_RESET, (void *)1);
