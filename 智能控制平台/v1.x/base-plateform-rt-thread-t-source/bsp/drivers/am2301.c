...
void updata_tmprh_limen(volatile struct temp_rh_limen *t_rh);

volatile struct temp_rh_limen t_rh_value;
...
void thread_am2301_entry(void* parameter)
{
	...
	while (1) {
		...
		if (is_bit_set(nms_if.trap_enable_bits, NMS_TRAP_ENABLE_BIT)){		//系统总告警是否开
			if (0 == (nms_if.trap_enable_bits & NMS_TRAP_TMP_EXC_EN_BIT)) {
				if((tmp > t_rh_value.temp_h) || (tmp < t_rh_value.temp_l))
					snmp_yeejoin_trap(YTI_TEMP_OUTOF_GAUGE);
			}

			if (0 == (nms_if.trap_enable_bits & NMS_TRAP_RH_EXC_EN_BIT))
				if((rh > t_rh_value.rh_h) || (rh < t_rh_value.rh_l))
					snmp_yeejoin_trap(YTI_RH_OUTOF_GAUGE);
...
int read_temp_rh_limen(volatile struct temp_rh_limen *t_rh)
{
	if (NULL == t_rh)
		return FAIL;

	return read_syscfgdata_tbl_from_cache(SYSCFGDATA_TBL_TMP_RH, 0, (void *)t_rh);
}

int save_temp_rh_limen(volatile struct temp_rh_limen *t_rh)
{
	if (NULL == t_rh)
		return FAIL;

	return write_syscfgdata_tbl(SYSCFGDATA_TBL_TMP_RH, 0, (void *)t_rh);
...
void updata_tmprh_limen(volatile struct temp_rh_limen *t_rh)
{
	...
	t_rh_value.temp_l = t_rh->temp_l;
	t_rh_value.temp_h = t_rh->temp_h;
	t_rh_value.rh_l = t_rh->rh_l;
	t_rh_value.rh_h = t_rh->rh_h;
	save_temp_rh_limen(&t_rh_value);
}
...
#define DELAY_COLSE_ELOCK_TIME (60*4)
...
int check_if_has_warning(int tick_cnt)
{
	...
	if (is_elock_open(ELOCK_PORT, ELOCK_PIN)) {
		if (!pin_in_is_set(ELOCK_SWITCH_PORT, ELOCK_SWITCH_PIN)) { /* 箱门是关闭的 */
			if((sys_info.delay_auto_elock_time <= 35)&&(sys_info.delay_auto_elock_time > 0)) {
				...
				} else if (tick_cnt-auto_close_lock_cnt > sys_info.delay_auto_elock_time*DELAY_COLSE_ELOCK_TIME) {
