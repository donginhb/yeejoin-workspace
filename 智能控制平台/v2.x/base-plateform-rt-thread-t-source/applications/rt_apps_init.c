...
void rt_sys_misc_entry(void* param)
{
	...
	volatile rt_uint32_t *p = (rt_uint32_t *)PSRAM_START_ADDR;
	rt_uint32_t i = 0;
#endif
	...
	if ((0 == (cnt & 0x7)) && (p < PSRAM_START_ADDR+PSRAM_MAX_LEN)) {
		if (0 != i) {
			*p = i++;
	...
	if ((0 == (cnt & 0x7)) && (p == PSRAM_START_ADDR)) {
		for (i=0; i<(PSRAM_MAX_LEN/4); ++i) {
			*p = i;
			if (*p != i)
		...
		printf_syn("check psram over! last addr:0x%x, last value:0x%x, i:0x%x\n", (p-1), *(p-1), i);
	}
#endif		
...
int rt_application_init(void)
{
	...
	th_handle = rt_thread_create("misc", rt_sys_misc_entry, RT_NULL,
	                           320, RT_THREAD_PRIORITY_MAX-2, 8);
	if (th_handle != RT_NULL)
		rt_thread_startup(th_handle);
#endif
