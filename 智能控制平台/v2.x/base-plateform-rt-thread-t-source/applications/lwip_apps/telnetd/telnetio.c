    ...
    if (0!=size && NULL!=buffer) {
        ...
        while (cnt > 0) {
            if ('\n'!=*pch) {
                ...
                if (2 != rb_write(&p->tx_rb_buf, (unsigned char*)"\r\n", 2))
                    break;
            }
            ++pch;
            --cnt;
        ...
        if (0!=(size -= cnt) && is_bit_clr(p->flag, IS_SEND_MAIL_FROM_SHELL2TELNET_BITMASK)) {
            if (RT_EOK == rt_mb_send(&tel_shell_ipc.telnet_shell_mb, (rt_uint32_t)dev))
                set_bit(p->flag, IS_SEND_MAIL_FROM_SHELL2TELNET_BITMASK);
...
__inline__ int telnetio_dev_no_get()
{
    return telio_cnt;
}
...
void try_flush_telio_txbuf(void)
{
    ...
    do {
        ...
        if (RT_EOK==ret_err && NULL!=dev) {
            ...
            pcb = dev->user_data;
            p   = pcb->callback_arg;
            clr_bit(p->flag, IS_SEND_MAIL_FROM_SHELL2TELNET_BITMASK);
            dev = NULL;
        } else {
            break;
        }
