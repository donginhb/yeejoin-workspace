...
#define PAGE_NUM_OF_ONCHIP_FLASH	((STM32_FLASH_SIZE)/(SIZE_PER_FLASH_PAGE/1024))
#endif

#define LAST_PAGE_NUM_OF_ONCHIP_FLASH	(PAGE_NUM_OF_ONCHIP_FLASH - PAGE_NUM_USED_BY_SYSCFGDATA_TBL)
...
#define SYSCFGDATA_TBL_PREV_SIZE_OF_FLASH	(sizeof(struct syscfgdata_tbl_prev))
...
#define SYSCFGDATA_TBL_MAGIC_NUM  	(0X5A5A6394UL)
#elif (EM_MULTI_MASTER_DEV || EM_MULTI_SLAVE_DEV)
#define SYSCFGDATA_TBL_MAGIC_NUM  	(0X5A5A6395UL)
#else
...
#define SYSCFG_DATA_DEBUG(x) rt_kprintf x
...
static int read_whole_syscfgdata_tbl(void *data, rt_size_t size);
static int
...
static int set_systbl_to_default_value(struct syscfgdata_tbl *p)
{
	int i;
#if RT_USING_GPRS
	char TELNUM[12] = {1,5,8,8,8,8,8,6,6,6,6};
#endif
	if (NULL == p) {
...
#if RT_USING_GPRS
	/* gprs_info */
	p->gprs_info.server_ipaddr.addr = WEBM_P_SERVER_DEFAULT_IP;
	p->gprs_info.server_port = WEBM_P_SERVER_DEFAULT_PORT;
	p->gprs_info.server_netmask.addr = DEFAULT_NET_MASK;
	p->gprs_info.server_gw.addr = WEBM_P_SERVER_DEFAULT_IP;
	p->gprs_info.prot_type = WEBM_P_SERVER_DEFAULT_PROTO;

	rt_strncpy(p->gprs_info.master_telnum,  TELNUM,	sizeof(p->gprs_info.master_telnum)-1);
	rt_strncpy(p->gprs_info.sms_telnum, TELNUM, sizeof(p->gprs_info.sms_telnum)-1);
	rt_strncpy(p->gprs_info.apn_name,  "3gwap",	USR_NAME_LEN_MAX+1);
	rt_strncpy(p->gprs_info.apn_usr,  "", USR_NAME_LEN_MAX+1);
	rt_strncpy(p->gprs_info.apn_pw, "",	PW_LEN_MAX+1);
#endif


	/* tw_papam */
	p->tw_param.sync_word = LIGHT300_MASTER_SLAVE_DEFAULT_SYN_WORD;
	p->tw_param.mod_type = GFSK;
	p->tw_param.mch_code = DISABLE;
	p->tw_param.center_freq = 470; /*unit MHZ*/
	p->tw_param.channal_num = 8;
	p->tw_param.Tx_dev = 4800;   /*unit HZ*/
	p->tw_param.Rx_dev = 4800;
	p->tw_param.Rx_bw = 51200;  /*unit HZ*/
	p->tw_param.Tx_rate = 9600;  /*unit bps*/
	p->tw_param.Rx_rate = 9600;   /*unit kbps*/

	p->tw_param.Tx_power = 10;  /*unit mw*/
	p->tw_param.Rx_sensitivity = -110;  /*unit dBm*/
	p->tw_param.alarm_mask = 0;

	p->tw_param.channel_no	= 0;
	p->tw_param.cid		= LIGHT300_MASTER_SLAVE_CID;
	p->tw_param.slave_id	= INVALID_WIRELESS_MAC_ID;


#if WIRELESS_MASTER_NODE
	/* white_register初始值均为0, 不用再赋值 */
#endif
	/* gw_em_info初始值均为0, 不用再赋值 */
...
get_member_start_and_size(int ind_m, int ind_s, struct syscfgdata_tbl *p, char **ps, int *cnt)
{
...
#if RT_USING_GPRS
	case SYSCFGDATA_TBL_GPRSCFG:
		*ps  = SYSCFGDATA_TBL_MEMBER_ADDR(p, gprs_info);
		*cnt = sizeof(p->gprs_info) >> 2;
		break;
#endif

	case SYSCFGDATA_TBL_TW_INFO:
		*ps  = SYSCFGDATA_TBL_MEMBER_ADDR(p, tw_param);
		*cnt = sizeof(p->tw_param) >> 2;
		break;

#if PT_DEVICE || CT_DEVICE
	case SYSCFGDATA_TBL_REGISTER_WHITER:
		*ps  = SYSCFGDATA_TBL_MEMBER_ADDR(p, white_register);
		*cnt = sizeof(p->white_register) >> 2;
		break;
#endif

	case SYSCFGDATA_TBL_GW_EM_INFO:
		*ps  = SYSCFGDATA_TBL_MEMBER_ADDR(p, gw_em_info);
		*cnt = sizeof(p->gw_em_info) >> 2;
		break;

#if EM_ALL_TYPE_BASE
	case SYSCFGDATA_TBL_WLM_485S_INFO:
		*ps  = SYSCFGDATA_TBL_MEMBER_ADDR(p, wl_master_info);
		*cnt = sizeof(p->wl_master_info) >> 2;
		break;

	case SYSCFGDATA_TBL_S_EMM_INFO:
		*ps  = SYSCFGDATA_TBL_MEMBER_ADDR(p, s_emm_info);
		*cnt = sizeof(p->s_emm_info) >> 2;
		break;
	...
	case SYSCFGDATA_TBL_EM_CONFIG_INFO:
		*ps  = SYSCFGDATA_TBL_MEMBER_ADDR(p, em_config_info);
		*cnt = sizeof(p->em_config_info) >> 2;
		break;
#endif
 ...
 * 该函数只有在数据库有效时，才调用
 * */
static void update_db_if_need(struct syscfgdata_tbl *p)
{
	rt_uint32_t cur_dbver, prev_dbver, real_dbver;

	cur_dbver  = MK_VER(M3_DB_VERSION, M3_DB_SUBVERSION, M3_DB_REVISION);
	prev_dbver = MK_VER(M3_DB_PREV_VERSION, M3_DB_PREV_SUBVERSION, M3_DB_PREV_REVISION);
	real_dbver = p->m3_sys_info.db_ver;

	SYSCFG_DATA_DEBUG(("%s(), line:%d\n", __func__, __LINE__));

	if (cur_dbver == real_dbver) {
		/* 数据库是最新的版本 */
		SYSCFG_DATA_DEBUG(("%s(), line:%d\n", __func__, __LINE__));
		return;
	} else if (prev_dbver == real_dbver) {
		SYSCFG_DATA_DEBUG(("%s(), line:%d\n", __func__, __LINE__));
		/* 数据库不是最新的版本，但是，可以升级 */
		do_update_db(p);
	} else {
		/* 数据库不是最新的版本，但是，不可以升级 */
		rt_kprintf("func:%s, database ver-%d.%d.%d is too old, "
				"prev_dbver-%d.%d.%d can update to cur_dbver-%d.%d.%d\n", __FUNCTION__,
				real_dbver>>16 & 0xff, real_dbver>>8 & 0xff, real_dbver & 0xff,
				M3_DB_PREV_VERSION, M3_DB_PREV_SUBVERSION, M3_DB_PREV_REVISION,
				M3_DB_VERSION, M3_DB_SUBVERSION, M3_DB_REVISION);
		while(1)
			;
	}
...
static int read_whole_syscfgdata_tbl(void *data, rt_size_t size)
{
	...
	if (size & 0x3) {
		rt_kprintf("NOTE:%s(), size(%x) not alignment\n", __func__, size);
	}

	p1 = (u32_t *)data;
	p2 = SYSCFGDATA_TBL_BASE_OF_FLASH;
	len = size > SYSCFGDATA_TBL_SIZE_OF_FLASH ? SYSCFGDATA_TBL_SIZE_OF_FLASH : size;
	len >>= 2;
	for (i=0; i<len; ++i) {
		*p1++ = *p2++;
	}
