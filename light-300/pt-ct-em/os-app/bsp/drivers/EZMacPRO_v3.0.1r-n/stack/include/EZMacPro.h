...
#define WAKE_UP_ERROR_NOT_TIMEOUT       0x8E
...
extern MacParams EZMacPRO_Init(void);
extern MacParams EZMacPRO_Wake_Up(void);
extern MacParams EZMacPRO_Sleep(void);
extern MacParams EZMacPRO_Idle(void);
extern MacParams EZMacPRO_Transmit(void);
extern MacParams EZMacPRO_Receive(void);
extern MacParams EZMacPRO_Idle(void);
extern MacParams EZMacPRO_Reg_Write(MacRegs name, U8);
extern MacParams EZMacPRO_Reg_Read (MacRegs name, U8 *value);
extern MacParams EZMacPRO_TxBuf_Write(ubase_t, VARIABLE_SEGMENT_POINTER(payload, U8, BUFFER_MSPACE));
extern MacParams EZMacPRO_RxBuf_Read(VARIABLE_SEGMENT_POINTER(length, ubase_t, BUFFER_MSPACE)
			,VARIABLE_SEGMENT_POINTER(payload, U8, BUFFER_MSPACE));
extern MacParams EZMacPRO_Ack_Write(ubase_t length, VARIABLE_SEGMENT_POINTER(payload, U8, BUFFER_MSPACE));

#define SET_EZMACPRO_MSR_USE_MACR 0
#if SET_EZMACPRO_MSR_USE_MACR
#define set_ezmacpro_msr(state) EZMacProReg.name.MSR=state
#else
extern base_t set_ezmacpro_msr(U8 state);
#endif
extern U8     get_ezmacpro_msr(void);
extern U8     get_ezmacpro_rssi(void);
