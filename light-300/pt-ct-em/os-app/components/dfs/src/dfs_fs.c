...
#define DPT_ITEM_SIZE	16		/* partition item size */
	...
	if (type != 0) {
		...
		/* get partition offset and size, use block as unit */
		part->offset = *(dpt+8)  | *(dpt+9)<<8  | *(dpt+10)<<16 | *(dpt+11)<<24;
		part->size   = *(dpt+12) | *(dpt+13)<<8 | *(dpt+14)<<16 | *(dpt+15)<<24;

		rt_kprintf("found part[%d], begin block:%u(%u), size block:%u(%u), size:", pindex,
				   part->offset, part->offset * 512, part->size, part->size * 512);

		if ((part->size>>11) > 0) { /* MB */
			unsigned int part_size;
			part_size = part->size >> 11;/* MB */
			if ((part_size>>10) > 0) { /* GB */
				rt_kprintf("%d.%d%s", part_size>>10, (part_size&0x3FF)*1000/1024,       "GB\r\n"); /* GB */
			} else {
				rt_kprintf("%d.%d%s", part_size,     ((part->size>>1)&0x3FF)*1000/1024, "MB\r\n"); /* MB */
			}
		} else {
...
void mkfs_mu(const char *fs_name, const char *device_name)
{
	if (NULL==device_name || NULL==fs_name) {
		rt_kprintf("param error\n");
		return;
	}

	if (0 == dfs_mkfs(fs_name, device_name)) {
		rt_kprintf("make filesystem succ\n");
		flush_sfwb(0);

		/* mount spi-flash fat partition 1 as root directory */
		if (dfs_mount(device_name, "/", fs_name, 0, 0) == 0)
			rt_kprintf("mount File System succ!\n");
		else
			rt_kprintf("mount File System fail!\n");
	} else {
		rt_kprintf("make filesystem fail\n");
	}

	return;
}
FINSH_FUNCTION_EXPORT(mkfs_mu, make a file system);
