...
#define LOGIN_NAME_INPUT_MAX (16)
#define LOGIN_PW_INPUT_MAX   (16)

enum login_state {
	LS_WAIT_LOGIN_IDLE = 0,
	LS_WAIT_LOGIN_NAME,
	LS_WAIT_LOGIN_PW,
	LS_LOGIN_SUCC,
};

static enum login_state login_proc_state;
int is_had_login;
static int prev_ch;

/* extern volatile int enable_console_print; */

static int shell_login_proc(int ch);
#endif

/* finsh thread */
static struct rt_thread finsh_thread;
ALIGN(RT_ALIGN_SIZE)
static char finsh_thread_stack[FINSH_THREAD_STACK_SIZE * 2];
...
void finsh_set_device(const char* device_name)
{
	...
	if (dev != RT_NULL && rt_device_open(dev, RT_DEVICE_OFLAG_RDWR) == RT_EOK) {
		...
		is_had_login = 0;
		login_proc_state = LS_WAIT_LOGIN_IDLE;
		prev_ch = '\0';
...
static int shell_login_proc(int ch)
{
	static char login_name_input[LOGIN_NAME_INPUT_MAX];
	static char login_pw_input[LOGIN_PW_INPUT_MAX];
	int len;
	struct telnetio_dev *ptel_dev;
	/*
		printf_syn("ch:%d, prev_ch:%d, login_proc_state:%d, is_had_login:%d, login_name_input[0]:%d\n",
			ch, prev_ch, login_proc_state, is_had_login, login_name_input[0]);
	*/
	if (LS_WAIT_LOGIN_IDLE==login_proc_state && ('\r'==ch || '\n'==ch)) {
		login_proc_state = LS_WAIT_LOGIN_NAME;
	}
	...
	switch (login_proc_state) {
	case LS_WAIT_LOGIN_NAME:
		if ((ch == '\r' || ch == '\n') && '\0'==login_name_input[0]) {
			/* enable_console_print = 1; */
			printf_syn("login:");
			goto ret_entry;
		} else if (ch == '\r' || ch == '\n') {
			printf_syn("\npassword:");
			login_proc_state = LS_WAIT_LOGIN_PW;
		} else {
			len = rt_strlen(login_name_input);
			len = MIN(len, LOGIN_NAME_INPUT_MAX-1);
			login_name_input[len++] = ch;
			len = MIN(len, LOGIN_NAME_INPUT_MAX-1);
			login_name_input[len] = '\0';
			printf_syn("%c", ch);
		}
		break;

	case LS_WAIT_LOGIN_PW:
		if (ch == '\r' || ch == '\n') {
			/* printf_syn("\ninput name:%s, input pw:%s\n", login_name_input, login_pw_input); */
			if (is_usr_pw_matching(login_name_input, login_pw_input)) {
				login_proc_state = LS_LOGIN_SUCC;
				is_had_login	 = 1;
				rt_kprintf("\nWelcome to ysh!\n"FINSH_PROMPT);
				prev_ch          = '\0';
				login_name_input[0] = '\0';
				login_pw_input[0]   = '\0';
				...
				rt_kprintf("line:%d, dev name:%s\n", __LINE__, shell->device->parent.name);

				if (0 == rt_strncmp("teln", shell->device->parent.name, 4)) {
					ptel_dev = container_of(shell->device, struct telnetio_dev, dev);
					ptel_dev->state = TELS_LOGIN_NAME;
				}
				printf_syn("\nlogin:");
				login_name_input[0] = '\0';
				login_pw_input[0]   = '\0';
			...
			len = rt_strlen(login_pw_input);
			len = MIN(len, LOGIN_NAME_INPUT_MAX-1);
			login_pw_input[len++] = ch;
			len = MIN(len, LOGIN_NAME_INPUT_MAX-1);
			login_pw_input[len] = '\0';
			printf_syn("*");
		}
...
int exitsh(void)
{
	printf_syn("Thank you for using ysh!\n");
	/* enable_console_print = 0; */
	is_had_login = 0;
	login_proc_state = LS_WAIT_LOGIN_IDLE;
...
FINSH_FUNCTION_EXPORT(exitsh, exit shell);
