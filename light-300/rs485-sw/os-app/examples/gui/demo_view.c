...
#define DVIEW_DEBUG(x) /* rt_kprintf x */
...
void demo_view_next(struct rtgui_widget* widget, rtgui_event_t *event)
{
	...
	DVIEW_DEBUG(("next view %d/%d\r\n", demo_view_current, demo_view_number));
}
...
void demo_view_prev(struct rtgui_widget* widget, rtgui_event_t *event)
{
	if (demo_view_current != 0)
		...
		demo_view_current = demo_view_number - 1;

	rtgui_view_show(demo_view_list[demo_view_current], RT_FALSE);

	DVIEW_DEBUG(("prev view %d/%d\r\n", demo_view_current, demo_view_number));
}
...
rtgui_view_t* demo_view(rtgui_workbench_t* workbench, const char* title)
{
	...
	DVIEW_DEBUG(("creat %dth view '%s' succ\r\n", demo_view_number, title));
	...
	/* 添加下一个视图和前一个视图按钮 */
	{
		...
		DVIEW_DEBUG(("next button (%d, %d), (%d, %d)\r\n", rect.x1, rect.y1, rect.x2, rect.y2));
		...
		DVIEW_DEBUG(("prev button (%d, %d), (%d, %d)\r\n", rect.x1, rect.y1, rect.x2, rect.y2));
		/* 创建"上一个"按钮 */
