...
void syn_sysdata(void)
{
#ifdef RT_USING_DFS
	flush_sfwb(0); /* mark by David */
#endif
	syscfgdata_syn_proc();
	rt_kprintf("syn sys-data and sys-cfg-data over!\n");

	return;
}

FINSH_FUNCTION_EXPORT(syn_sysdata, syn_sysdata);

void reset_system(void)
{
	syn_sysdata();

	rt_kprintf("Will reset system!\n");
	reset_whole_system();

	return;
}
FINSH_FUNCTION_EXPORT(reset_system, reset system);
#endif

#if 0
extern volatile int sst25_w_cnt, sst25_w_bytes;
extern volatile unsigned long sst25_w_addr;
long hello(void)
{
	rt_kprintf("Hello RT-Thread! int sst25_w_cnt:%d, sst25_w_bytes:%d, sst25_w_addr:%u\n",
			   sst25_w_cnt, sst25_w_bytes, sst25_w_addr);
...
static long _list_thread(struct rt_list_node* list)
{
	...
	rt_kprintf(" thread  pri  status      sp     stack size    max used     left tick  error use ticks\n");
	rt_kprintf("-------- ---- ------- ---------- ---------- --------------- ---------- ----- ----------\n");
	for (node = list->next; node != list; node = node->next) {
		...
		else rt_kprintf("%8d", thread->stat);

		ptr = (rt_uint8_t*)thread->stack_addr;
		while (*ptr == '#')ptr ++;

		rt_kprintf(" 0x%08x 0x%08x 0x%08x(%02d\%) 0x%08x %03d %10d\n",
				   thread->stack_size + ((rt_uint32_t)thread->stack_addr - (rt_uint32_t)thread->sp),
				   thread->stack_size,
				   thread->stack_size - ((rt_uint32_t) ptr - (rt_uint32_t)thread->stack_addr),
				   (thread->stack_size - ((rt_uint32_t) ptr - (rt_uint32_t)thread->stack_addr))*100/thread->stack_size,
				   thread->remaining_tick,
				   thread->error, thread->use_ticks);

	}

	rt_kprintf("all ticks:%u\n", rt_tick_get());
...
void set_debug(int mod, int item, int on_off)
{
	switch (mod) {
	case 1:
		switch (item) {
		case 0:
			printf_syn("print debug vector is 0x%x\n", sinkinfo_print_switch);
			break;
		...
		case 1:
			if (0 != on_off) {
				...
				clr_bit(sinkinfo_print_switch, SINKINFO_7880_GETDATA_FAIL_BIT);
			}
			break;
		...
		case 2:
			if (0 != on_off) {
				...
				clr_bit(sinkinfo_print_switch, SINKINFO_EM_GETDATA_FAIL_BIT);
			}
			break;
	...
	default:
		printf_syn("invalid mod(%d)\n", mod);
		break;
	}

	return;
}
FINSH_FUNCTION_EXPORT(set_debug, mod item on_off)
#endif
