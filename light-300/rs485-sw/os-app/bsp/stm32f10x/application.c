...
#define TEST_TL16_485	0
#define PHONY_EM_ECHO	0

#define CREAT_THREAD_FOR_TEST 0
...
static int read_data_from_tl16(enum lt_485if_e rs485_if, int *had_proc)
{
	rt_device_t dev;
	int recv_data_len, i;
	...
	recv_data_len = recv_data_by_tl16_485(dev, tl16_buf, sizeof(tl16_buf));

	lt485_log(("%s() read %d bytes from rs485_if-%d\n", __func__,
			recv_data_len, rs485_if));

	for (i=0; i<recv_data_len; ++i) {
		printf_syn("lt_if-%d:0x%02x\n", rs485_if, tl16_buf[i]);
		++*had_proc;
	}
...
static void rt_test_tl16_485_entry(void* parameter)
{
	rt_uint32_t e;
	rt_err_t err;
	int had_proc;
	...
	while (1) {
		...
		lt485_log(("%s() recv event 0x%x\n", __func__, e));

		do {
			had_proc = 0;

			if (is_bit_set(e, EVENT_BIT_TL16_1_RECV_DATA))
				read_data_from_tl16(LT_485_TL16_U1, &had_proc);

			if (is_bit_set(e, EVENT_BIT_TL16_2_RECV_DATA))
				read_data_from_tl16(LT_485_TL16_U2, &had_proc);

			if (is_bit_set(e, EVENT_BIT_TL16_3_RECV_DATA))
				read_data_from_tl16(LT_485_TL16_U3, &had_proc);

			if (is_bit_set(e, EVENT_BIT_TL16_4_RECV_DATA))
				read_data_from_tl16(LT_485_TL16_U4, &had_proc);

			if (is_bit_set(e, EVENT_BIT_TL16_5_RECV_DATA))
				read_data_from_tl16(LT_485_TL16_U5, &had_proc);

			if (is_bit_set(e, EVENT_BIT_TL16_6_RECV_DATA))
				read_data_from_tl16(LT_485_TL16_U6, &had_proc);

			if (is_bit_set(e, EVENT_BIT_TL16_7_RECV_DATA))
				read_data_from_tl16(LT_485_TL16_U7, &had_proc);

			if (is_bit_set(e, EVENT_BIT_TL16_8_RECV_DATA))
				read_data_from_tl16(LT_485_TL16_U8, &had_proc);
		} while (had_proc);
	} /* while(1) */
...
static void rt_test_tl16_485_use_emc_if_entry(void* parameter)
{
	void *ptr;
	int send_data_len, i;
	unsigned char buf[] = {
			0xfe, 0xfe, 0xfe, 0xfe, 0x68,
			0x47, 0x54, 0x00, 0x12, 0x00,
			0x00, 0x68, 0x01, 0x02, 0x65,
			0xf3, 0xd8, 0x16};

	ptr = get_us485_dev_ptr(LT_485_EMC_UART, 1);

	while (1) {
		printf_syn("\n%s() will send frame:\n", __func__);
		for (i=0; i<sizeof(buf); ++i)
			printf_syn("0x%02x ", buf[i]);

		printf_syn("\n\n");

		send_data_len = send_data_by_485(ptr, buf, sizeof(buf));

		if (sizeof(buf) != send_data_len) {
			printf_syn("%s() send_data_len is err(%d/%d)\n", __func__,
					send_data_len, sizeof(buf));
		}
...
static void rt_test_phony_em_echo_entry(void* parameter)
{
	void *ptr;
	rt_err_t err;
	rt_uint32_t e;
	int send_data_len, recv_data_len, i;
	unsigned char buf[128];

	ptr = get_us485_dev_ptr(LT_485_EMC_UART, 1);

	while (1) {
		err = rt_event_recv(&em_protocol_data_event_set, EVENT_BIT_UART485_2_RECV_DATA,
				RT_EVENT_FLAG_OR | RT_EVENT_FLAG_CLEAR, RT_WAITING_FOREVER, &e);
		...
		if (RT_EOK == err) {
			do {
				...
				printf_syn("\n%s() will send echo %d byte(s):\n", __func__, recv_data_len);
				for (i=0; i<recv_data_len; ++i)
					printf_syn("0x%02x ", buf[i]);
				printf_syn("\n\n");

				send_data_len = send_data_by_485(ptr, buf, recv_data_len);
			...
			printf_syn("%s() recv event fail(%d)\n", __func__, err);
		}
...
void rt_init_thread_entry(void* parameter)
{
	...
	rt_device_t rtc_dev;
#endif
#if TEST_TRAP || EM_ALL_TYPE_BASE || TEST_485 || TEST_TL16_485 || PHONY_EM_ECHO
	...
	rt_kprintf("tl16c554 reset over\n");
	rt_hw_tl16_usart_init();

#if (RT_USING_RS485_BUS && !TEST_485 && !CREAT_THREAD_FOR_TEST)
	init_sys_485();
#endif

#if 1 && !TEST_TL16_485 && !PHONY_EM_ECHO && !CREAT_THREAD_FOR_TEST	/* mark by David */
...
#if 0
	rt_485sw_em_init();
	uart_485_tl16_1_dev_init();
	uart_485_tl16_2_dev_init();
	uart_485_tl16_3_dev_init();
	uart_485_tl16_4_dev_init();
	uart_485_tl16_5_dev_init();
	uart_485_tl16_6_dev_init();
	uart_485_tl16_7_dev_init();
	uart_485_tl16_8_dev_init();
#endif

#if TEST_TL16_485
	thread_h = rt_thread_create("tl16-485", rt_test_tl16_485_entry, RT_NULL, 512, 30, 50);
	if (thread_h != RT_NULL)
		rt_thread_startup(thread_h);

	thread_h = rt_thread_create("emc-p485", rt_test_tl16_485_use_emc_if_entry, RT_NULL, 512, 30, 50);
	if (thread_h != RT_NULL)
		rt_thread_startup(thread_h);
...
#if PHONY_EM_ECHO
	thread_h = rt_thread_create("phony-em", rt_test_phony_em_echo_entry, RT_NULL, 512+512, 30, 50);
	if (thread_h != RT_NULL)
		rt_thread_startup(thread_h);
#endif
...
#if !CREAT_THREAD_FOR_TEST
extern int do_tl16_irq(enum tl16_x_csx_addr_e x_csx, int *had_proc);
...
static void rt_isr_event_entry(void* parameter)
{
	...
	while (1) {
		err = rt_event_recv(&isr_event_set
				, EVENT_BIT_NEED_RUN_TL16_1_IRQ | EVENT_BIT_NEED_RUN_TL16_2_IRQ
				, RT_EVENT_FLAG_OR | RT_EVENT_FLAG_CLEAR, RT_WAITING_FOREVER, &e);
		...
		if (RT_EOK != err) {
			...
			do {
				had_proc = 0;
				if (is_bit_set(e, EVENT_BIT_NEED_RUN_TL16_1_IRQ)) {
					err = rt_mutex_take(&tl16_x_lock, RT_WAITING_FOREVER);
					if (RT_EOK == err) {
						for (x_csx=TL16_1_CSA_ADDR; x_csx<=TL16_1_CSD_ADDR; ++x_csx) {
							do_tl16_irq(x_csx, &had_proc);
						}
						rt_mutex_release(&tl16_x_lock);
					} else {
						printf_syn("%s(), get tl16_x_lock fail(%d)\n", __func__, err);
					}

				}

				if (is_bit_set(e, EVENT_BIT_NEED_RUN_TL16_2_IRQ)) {
					err = rt_mutex_take(&tl16_x_lock, RT_WAITING_FOREVER);
					if (RT_EOK == err) {
			...
			} while ((had_proc || pin_in_is_set(tl16c554_1_intx_gpio, tl16c554_1_intx_pin)
					|| pin_in_is_set(tl16c554_2_intx_gpio, tl16c554_2_intx_pin))
					&& (rt_tick_get() - temp_tick < get_ticks_of_ms(1000)));
		}
	}
...
static void rt_tst_1_entry(void* parameter)
{
	rt_err_t err;
	...
	while (1) {
		...
		err = rt_mutex_take(&test_mutex, get_ticks_of_ms(2000));
		if (RT_EOK != err) {
			printf_syn("%s() get mutex fail(%d)\n", __func__, err);
		} else {
			if (rt_thread_self() != test_mutex.owner) {
				printf_syn("PANIC:%d, %s(), self:%s(%d,%d,%d), owner:%s(%d, %d, %d), %d, %d, %d\n",
						...
						test_mutex.owner->init_priority,
						test_mutex.owner->current_priority,
						test_mutex.owner->error,
						test_mutex.hold, test_mutex.original_priority,
						test_mutex.value);
			}

			printf_syn("=============================%s(), line:%d, had get mutex, will release...\n", __func__, __LINE__);
			rt_thread_delay(get_ticks_of_ms(500));

			err = rt_mutex_release(&test_mutex);
			if (RT_EOK != err) {
				printf_syn("th:%s, line:%d, %s() release mutex fail(%d), owner:%s\n",
...
static void rt_tst_2_entry(void* parameter)
{
	rt_err_t err;


	while (1) {
		rt_thread_delay(get_ticks_of_ms(2900));
...
extern struct rt_mutex print_mutex;
...
int rt_application_init()
{
	...
	rt_mutex_init(&print_mutex, "prtmux", RT_IPC_FLAG_PRIO);
#if RT_USING_FILESYSTEM
	rt_sem_init(&spiflash_sem, "sfsem", 1, RT_IPC_FLAG_PRIO);
#endif
	rt_mutex_init(&tl16_x_lock, "tl16-x", RT_IPC_FLAG_PRIO);
...
#if !CREAT_THREAD_FOR_TEST
	thread_h = rt_thread_create("isr_e", rt_isr_event_entry, RT_NULL, 1024, ISR_EVENT_THREAD_PRIORITY, 10);
	if (thread_h != RT_NULL)
		rt_thread_startup(thread_h);
#endif
...
#if CREAT_THREAD_FOR_TEST
	rt_mutex_init(&test_mutex, "tst", RT_IPC_FLAG_PRIO);

	thread_h = rt_thread_create("tst-1", rt_tst_1_entry, RT_NULL, 1024, 11, 10);
	if (thread_h != RT_NULL)
		rt_thread_startup(thread_h);

	thread_h = rt_thread_create("tst-2", rt_tst_2_entry, RT_NULL, 1024, 12, 10);
	if (thread_h != RT_NULL)
		rt_thread_startup(thread_h);
#endif
	return 0;
