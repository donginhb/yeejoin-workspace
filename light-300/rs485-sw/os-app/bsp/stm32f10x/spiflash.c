...
static rt_size_t rt_spiflash_read(rt_device_t dev, rt_off_t pos, void* buffer, rt_size_t size)
{
	...
	sf_read_data((part.offset + pos)*RT_DFS_ELM_MAX_SECTOR_SIZE, buffer, size*RT_DFS_ELM_MAX_SECTOR_SIZE, 0);
...
static rt_size_t rt_spiflash_write (rt_device_t dev, rt_off_t pos, const void* buffer, rt_size_t size)
{
	...
	sf_write_data((part.offset + pos)*RT_DFS_ELM_MAX_SECTOR_SIZE, buffer, size*RT_DFS_ELM_MAX_SECTOR_SIZE);
