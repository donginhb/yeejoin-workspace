...
def Init_BuildOptions(root_directory):
	global Rtt_Root
	global BuildOptions
	...
	# parse rtconfig.h to get used component
	PreProcessor = SCons.cpp.PreProcessor()
	f = file(BuildOptions['APP_CFG_FILENAME'].strip()[1:-1], 'r')
	print BuildOptions['APP_CFG_FILENAME'].strip()[1:-1],'file process over!!'
	contents = f.read()
	f.close()
	PreProcessor.process_contents(contents)
	BuildOptions.update(PreProcessor.cpp_namespace)


	#print "PrepareBuilding() function......"
	#print Rtt_Root
	# parse rtconfig.h to get used component
	PreProcessor = SCons.cpp.PreProcessor()
	...
	def GetDepend(depend):
		...
		if type(depend) == type('str'):
			if not BuildOptions.has_key(depend) or BuildOptions[depend] == 0:
				...
				print 'A.GetDepend() %-40s result %s' %(depend,BuildOptions[depend])
				return BuildOptions[depend]
			else:
				print 'EA.GetDepend() %-40s' %(depend)
		elif type(depend) == type([1,2]):
			for item in depend:
				if type(item) == type('str'):
					if (not BuildOptions.has_key(item))  or BuildOptions[item] == 0:
						if len(item)!=0:
							building = False
					elif BuildOptions[item] != '':
						print 'B.GetDepend() %-40s result %s' %(item, BuildOptions[item])
						return BuildOptions[item]
					else:
						if item == 'RT_USING_LWIP':
							if BuildOptions.has_key('RT_USING_TCPIP_STACK'):
								if not BuildOptions['RT_USING_TCPIP_STACK']:
									building = False
					
						#	elif BuildOptions.has_key('RT_USING_TCPIP_STACK'):
						elif item =='RT_USING_DFS':
							if BuildOptions.has_key('RT_USING_FILESYSTEM'):
								if not BuildOptions['RT_USING_FILESYSTEM']:
									building = False
						elif item =='RT_USING_DFS_UFFS':
							if BuildOptions.has_key('RT_USING_UFFS'):
								if not BuildOptions['RT_USING_UFFS']:
									building = False
						elif item =='RT_USING_DFS_ELMFAT':
							if BuildOptions.has_key('RT_USING_ELMFAT'):
								if not BuildOptions['RT_USING_ELMFAT']:
									building = False

						elif item =='RT_USING_RTGUI':
							if BuildOptions.has_key('RT_USING_GUI'):
								if not BuildOptions['RT_USING_GUI']:
									building = False
						else:
							print 'EB.GetDepend() %-40s' %(item)
				else:
					print 'EC.GetDepend() %-40s' %(item)
		else:
			print '+.GetDepend() info:', type(depend)
	
		print 'C.GetDepend() %-40s result %s' %(depend, building)
		return building
		...
		print 'D.GetDepend() %-40s result %s' %(depend, building)
		return building
else:
	def GetDepend(depend):
		building = True
...
#	building = True
#	if type(depend) == type('str'):
#		print '++++ depend:',depend,'is string type!!'
#
#		if not BuildOptions.has_key(depend) or BuildOptions[depend] == 0:
#			print "** GetDepend() func:", depend," return false!"
#			building = False
#		elif BuildOptions[depend] != '':
#			building = BuildOptions[depend]
#		else:
#			print "depend[0:8]:", depend[0:8]
#			if 'RT_USING' == depend[0:8]:
#				building = True
#			else:
#				building = False #return false when null
#	else:
#		print '---- depend:',depend,'is not string type!!',type(depend)
#		# for list type depend
#		building = False
#		for item in depend:
#			if item != '':
#				if BuildOptions.has_key(item):
#					if BuildOptions[item] == 0:
#						pass
#					elif BuildOptions[item] == '':
#						print  'str(item)[0:8]:', str(item)[0:8]
#
#						if str('RT_USING') == str(item)[0:8]:
#							building = True
#						else:
#							pass
#					else:
#						building = BuildOptions[item]
#
#					print '****', item, 'is',BuildOptions[item],'return',building
#					break
#				else:
#					continue
#			else:
#				continue
#
#	print 'GetDepend()', depend, 'result', building
#	return building
#
...
def DefineGroup(name, src, depend, **parameters):
    ...
    is_define_grp = GetDepend(depend)
#    print  '*DefineGroup() %-40s is_define_grp:%s' %(depend,str(is_define_grp))
#    print name
    
    if not is_define_grp:
        return []
