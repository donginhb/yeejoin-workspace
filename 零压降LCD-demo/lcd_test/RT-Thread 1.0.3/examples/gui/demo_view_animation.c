...
void timeout(struct rtgui_timer* timer, void* parameter)
{
	...
	/* 绘图 */
	rtgui_dc_draw_text(dc, "亿江(北京)科技发展有限公司", &text_rect);
...
rtgui_view_t *demo_view_animation(rtgui_workbench_t* workbench)
{
	...
	rtgui_font_get_metrics(RTGUI_WIDGET_FONT(RTGUI_WIDGET(view)), "亿江(北京)科技发展有限公司", &text_rect);
	rtgui_rect_moveto(&text_rect, 0, 45);
	/* 启动定时器以触发动画 */
	timer = rtgui_timer_create(25, RT_TIMER_FLAG_PERIODIC, timeout, (void*)view);
