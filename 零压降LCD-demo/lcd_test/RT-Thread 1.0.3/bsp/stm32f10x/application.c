...
static void rt_sys_operation_entry(void* parameter)
{
	unsigned cnt=0;
	...
	while (1) {
		...
		blink_led(sys_led_CHANNEL_d4, sys_led_1hz, cnt);
		blink_led(sys_led_CHANNEL_d5, sys_led_2hz, cnt);
	}
...
int rt_application_init()
{
	...
	/* init led thread */  
	result = rt_thread_init(&misc_thread, "ml_misc", rt_sys_operation_entry, RT_NULL, (rt_uint8_t*)&misc_stack[0],
							sizeof(misc_stack), 0x19, 5);
	if (result == RT_EOK)
		rt_thread_startup(&misc_thread);
