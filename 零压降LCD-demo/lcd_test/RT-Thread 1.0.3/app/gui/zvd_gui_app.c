...
int app_view_structure(struct rtgui_workbench *workbench, struct rtgui_view *view)
{
...
#else  
	label = rtgui_label_create(MALOOEI_TITLE);
#endif
	...
	//sys_form = rtgui_form_create(sys_form_head, SYS_FORM_ROWS, SYS_FORM_COLS, &body_rect);
	//rtgui_notebook_add(zvd_notebook, "测试", RTGUI_WIDGET(sys_form));
	...
	if (NULL != rtc_dev) {
		...
		if (NULL != sys_time_textbox) {
			...
			RTGUI_WIDGET_FOREGROUND(RTGUI_WIDGET(sys_time_textbox)) = red;
			RTGUI_WIDGET_BACKGROUND(RTGUI_WIDGET(sys_time_textbox)) = green; //default_background;
...
int app_view_ml_structure(struct rtgui_workbench *workbench, struct rtgui_view *view)
{
	int ret = SUCC;
	rtgui_rect_t rect, body_rect;
	rtgui_label_t* label;
	...
	if (NULL != label) {
		RTGUI_WIDGET_TEXTALIGN(RTGUI_WIDGET(label)) = RTGUI_ALIGN_CENTER_HORIZONTAL | RTGUI_ALIGN_CENTER_VERTICAL;
		RTGUI_WIDGET_FOREGROUND(RTGUI_WIDGET(label)) = light_grey;
	...
	zvd_notebook = rtgui_notebook_create(&body_rect, RTGUI_NOTEBOOK_TOP);
	rtgui_container_add_child(RTGUI_CONTAINER(view), RTGUI_WIDGET(zvd_notebook));
	rtgui_widget_set_rect(RTGUI_WIDGET(zvd_notebook), &body_rect);
  
	re_form = rtgui_form_create(re_form_head, RE_FORM_ROWS, RE_FORM_COLS, &body_rect);
	rtgui_notebook_add(zvd_notebook, "Toolbar1", RTGUI_WIDGET(re_form));
	RTGUI_WIDGET_FOREGROUND(RTGUI_WIDGET(re_form)) = light_grey;
	RTGUI_WIDGET_BACKGROUND(RTGUI_WIDGET(re_form)) = black;
#endif  
...
#if 1
	se_form = rtgui_form_create(se_form_head, SE_FORM_ROWS, SE_FORM_COLS, &body_rect);
	rtgui_notebook_add(zvd_notebook, "Toolbar2", RTGUI_WIDGET(se_form));
	RTGUI_WIDGET_FOREGROUND(RTGUI_WIDGET(se_form)) = light_grey;
	RTGUI_WIDGET_BACKGROUND(RTGUI_WIDGET(se_form)) = black;
#endif
...
#if 1
	sys_form = rtgui_form_create(sys_form_head, SYS_FORM_ROWS, SYS_FORM_COLS, &body_rect);
	rtgui_notebook_add(zvd_notebook, "Toolbar3", RTGUI_WIDGET(sys_form));
	RTGUI_WIDGET_FOREGROUND(RTGUI_WIDGET(sys_form)) = light_grey;
	RTGUI_WIDGET_BACKGROUND(RTGUI_WIDGET(sys_form)) = black;
#endif  
...
#if 0 
	other_form = rtgui_form_create(other_form_head, OTHER_FORM_ROWS, OTHER_FORM_COLS, &body_rect);
	rtgui_notebook_add(zvd_notebook, "zw", RTGUI_WIDGET(other_form));
#endif
	...
	if (NULL != rtc_dev) {
		...
		if (NULL != sys_time_textbox) {
			RTGUI_WIDGET_TEXTALIGN(RTGUI_WIDGET(sys_time_textbox)) = RTGUI_ALIGN_CENTER_HORIZONTAL | RTGUI_ALIGN_CENTER_VERTICAL;
			RTGUI_WIDGET_FOREGROUND(RTGUI_WIDGET(sys_time_textbox)) = black;
			RTGUI_WIDGET_BACKGROUND(RTGUI_WIDGET(sys_time_textbox)) = white; //default_background;
			//RTGUI_WIDGET_BACKGROUND(RTGUI_WIDGET(view));  
			rtgui_widget_set_rect(RTGUI_WIDGET(sys_time_textbox), &body_rect);
			rtgui_container_add_child(RTGUI_CONTAINER(view), RTGUI_WIDGET(sys_time_textbox));
